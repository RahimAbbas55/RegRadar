# Scores answer relevancy using RAGAS 
import sys
import os
import json
...
if __name__ == "__main__":
    ...
    print(f"Scoring answer relevancy for {len(dataset)} in-scope questions...")
    result = evaluate(dataset, metrics=[answer_relevancy])
    scores_df = result.to_pandas()
    for id_, score in zip(ids, scores_df["answer_relevancy"]):
        print(f"{id_}: {score:.3f}")
    print(f"\nAverage answer relevancy: {scores_df['answer_relevancy'].mean():.3f}")